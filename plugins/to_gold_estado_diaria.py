...
try:
    spark = (
        SparkSession.builder.appName("to_gold_estado_diario")
...
def main():
    ...
    s3_gold_estado = f"s3a://{s3_bucket}/gold/foco_estado/diario/extract_year={year}/extract_month={month}/extract_day={day}"
    ...
    try:
        ...
        FROM
            df_referencia
        GROUP BY DATREF, DATVER, ESTADO
        ORDER BY QUANTIDADE_FOCO DESC
        """

        # Executar a query
        gold_foco_estado = spark.sql(foco_estado_agregado)

        # Reparticionar o DataFrame para evitar muitos arquivos pequenos no S3
        gold_foco_estado = gold_foco_estado.repartition(1)

        # Salvar gold_foco_estado no S3 (camada gold)
        gold_foco_estado.write.mode("overwrite").parquet(s3_gold_estado)
