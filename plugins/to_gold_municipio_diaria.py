...
try:
    spark = (
        SparkSession.builder.appName("to_gold_municipio_diario")
...
def main():
    ...
    s3_gold_municipio = f"s3a://{s3_bucket}/gold/foco_municipio/diario/extract_year={year}/extract_month={month}/extract_day={day}"
    ...
    try:
        ...
        FROM
            df_referencia
        GROUP BY DATREF, DATVER, ESTADO, MUNICIPIO
        ORDER BY QUANTIDADE_FOCO DESC
        """

        # Executar a query
        gold_foco_municipio = spark.sql(foco_municipio_agregado)

        # Reparticionar o DataFrame para evitar muitos arquivos pequenos no S3
        gold_foco_municipio = gold_foco_municipio.repartition(1)

        # Salvar gold_foco_municipio no S3 (camada gold)
        gold_foco_municipio.write.mode("overwrite").parquet(s3_gold_municipio)
